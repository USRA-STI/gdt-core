...
class TestRebinningBinned(unittest.TestCase):
    ...
    def test_combine_by_factor(self):
        res = combine_by_factor(counts, count_uncert, exposure, edges, 4)
        new_counts, new_uncert, new_exposure, new_edges = res
        ...
        self.assertCountEqual(new_uncert, np.sqrt([10.0, 26.0]))
        
        with self.assertRaises(AssertionError):
            combine_by_factor(counts, count_uncert, exposure, edges, 0)

    def test_rebin_by_time(self): 
        res = rebin_by_time(counts, count_uncert, exposure, edges, 4.096)
        new_counts, new_uncert, new_exposure, new_edges = res
        self.assertCountEqual(new_counts, np.array([10.0, 26.0]))
        self.assertCountEqual(new_exposure, np.array([4.024, 3.854]))
        self.assertCountEqual(new_edges, np.array([0.0, 4.096, 8.192]))
        ...
        with self.assertRaises(AssertionError):
            rebin_by_time(counts, count_uncert, exposure, edges, -1.0)
        
    def test_combine_into_one(self): 
        res = combine_into_one(counts, count_uncert, exposure, edges)
        new_counts, new_uncert, new_exposure, new_edges = res
        ...
        self.assertCountEqual(new_uncert, np.sqrt([36.0]))
    
    def test_rebin_by_edge_index(self):
        res = rebin_by_edge_index(counts, count_uncert, exposure, edges, index)
        new_counts, new_uncert, new_exposure, new_edges = res
        ...
        assert np.all(new_uncert.round(6) == np.sqrt([3.0, 7.0, 11.0, 15.0]).round(6))
        
    def test_rebin_by_edge(self):
        new_edges = np.array([0.0, 2.048, 4.096, 6.144, 8.192])
        res = rebin_by_edge(counts, count_uncert, exposure, edges, new_edges)
        new_counts, new_uncert, new_exposure, new_edges = res
        self.assertCountEqual(new_counts, np.array([3.0, 7.0, 11.0, 15.0]))
        self.assertCountEqual(new_exposure, np.array([2.034, 1.99, 2.044, 1.81]))
        self.assertCountEqual(new_edges, np.array([0.0, 2.048, 4.096, 6.144, 8.192]))
    ...
    def test_rebin_by_snr(self):
        ...
        res =  rebin_by_snr(counts, count_uncert, exposure, edges, back_counts, 3.0)
        new_counts, new_uncert, new_exposure, new_edges = res
        ...
        self.assertCountEqual(new_uncert, np.sqrt([10.0, 5.0, 6.0, 7.0, 8.0]))

        with self.assertRaises(ValueError):
            rebin_by_snr(counts, count_uncert, exposure, edges, -1.0*back_counts, 3.0)
